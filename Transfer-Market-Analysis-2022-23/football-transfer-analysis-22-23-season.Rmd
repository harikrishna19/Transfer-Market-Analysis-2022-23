...
# city_icon<-readPNG("city_icon.PNG")
# city_icon<-rasterGrob(city_icon,interpolate = T)
# 
# 
# ars_icon<-readPNG("download.PNG")
# ars_icon<-rasterGrob(ars_icon,interpolate = T)
...
gg_arsenal <- ggplot(ars_data, aes(x = Wk, y = Points, color = "Arsenal")) +
  ...
  scale_color_manual(values = c("Arsenal" = "red"))+theme_classic()
gg_city <- ggplot(mci_data, aes(x = Wk, y = Points, color = "Man City")) +
  ...
  scale_y_continuous(breaks = c(0, 1, 3)) +
  labs(x = "MatchWeek", y = "Wk", color = "Team") +
  scale_color_manual(values = c("Man City" = "blue"))+theme_classic()
...
What a title race:Terrific

3.Number of signing per pl team

4.Arsenal London Derbies record

5.NewCastle signings
...
Article to be out on 9/08/2023(Thursday)
